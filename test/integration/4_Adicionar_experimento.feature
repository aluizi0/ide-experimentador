...
Quando clico para adicionar os parâmetros (ID e nome) e confirmo
...
Dado que o usuário deseja registrar um novo robô
Quando clico para adicionar os parâmetros (ID e nome) e confirmo
Então o robô não é adicionado devido a um erro ou ao fornecimento de um ID/nome já existente.
...
Então o robô executa a operação como esperado.
...
Então o robô não executa a operação como esperado.
